...
for i=1:num_labels
  y_c = (y==i);
  J += sum((-y_c.*log(a3(:,i)) - (1-y_c).*log(1 - a3(:,i))))/m;
end

regularization = lambda / (2*m) * (Theta1_no_bias(:)'*Theta1_no_bias(:) + Theta2_no_bias(:)'*Theta2_no_bias(:));
J += regularization;
%fprintf(['Cost : %f \n'], J);
...
for i=1:m
  ...
  delta2(i,:) = (delta3(i,:)*Theta2_no_bias).*sigmoidGradient(z2(i,:));
